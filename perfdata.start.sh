...
echo "Performance data collection starting ... "
echo "Data collection interval	$DCINTERVAL seconds "
echo "Loop count			$NUMLOOPS iterations"
echo " "
...
tag "starting data collection at: "`date`" with delay = " $DCINTERVAL
...
do
  ...
  then
    ...
    tag "sar -bBcdqrRuvwWy -n ALL -P ALL" $DCINTERVAL "1 output ;"
    sar -bBcdqrRuvwWy -n ALL -P ALL $SARDELAY 1 >>${OUTFILE}
  else
    tag "sar -A" $DCINTERVAL "1 output ;"
